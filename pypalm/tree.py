...
def tree(exchangeability_blocks, design_matrix=None):
    ...
    permutation_tree[0], permutation_tree[2]=maketree(exchangeability_blocks[:,1:],design_matrix,O,wholeblock,wholeblock)
    if wholeblock:
...
def maketree(exchangeability_blocks, design_matrix, O, wholeblock=False, nosf=False):
    ...
    for u in range(nU):
        ...
        if exchangeability_blocks.shape[1] > 1:
            wholeblockb = exchangeability_blocks[np.where(idx)[0][0], 0] > 0 > 0
            ...
            elif permutation_tree[u][2].shape[1] > 1:
                if np.isnan(permutation_tree[u][0][0]):
                    permutation_tree[u][1] = np.array([])
                else:
                    permutation_tree[u][1] = np.zeros((len(permutation_tree[u][2]), 1))
            else:
                permutation_tree[u][1] = np.zeros((len(permutation_tree[u][3]), 1))
    ...
    if wholeblock and nU > 1:
        ...
        idx = np.argsort(S)
        S = S[idx]
        S= [S, np.arange(S.size), np.arange(S.size)]
